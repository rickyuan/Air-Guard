...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			graphwidth = (float) (width * graphwidthPers);

			if (horlabels == null) {
				horlabels = generateHorlabels(graphwidth);
			}
			if (verlabelsLeft == null) {
				verlabelsLeft = generateVerlabels(graphheight);
			}
			if (verlabelsRight == null) {
				verlabelsRight = generateVerlabels(graphheight);
			}
			// vertical lines
			paint.setTextAlign(Align.LEFT);

			// horizontal labels + lines
			horstart = (width - graphwidth) /2;
...
//				if (i == 0)
			...
			paint.setTextAlign(Align.CENTER);
			canvas.drawText(titleLeft==null?"":titleLeft, 0-10, border-18,
					paint);
			canvas.drawText(titleRight==null?"":titleRight, graphwidth  + horstart+10, border-18,
					paint);
			if (maxY == minY) {
		...
		private void onMoveGesture(float f) {
			...
			if (viewportSize != 0) {
				...
				if (!staticHorizontalLabels)
					horlabels = null;
				if (!staticVerticalLeftLabels)
					verlabelsLeft = null;
				if (!staticVerticalRightLabels)
					verlabelsRight = null;
		...
		protected void onDraw(Canvas canvas) {
			...
			if (getGraphViewStyle().getVerticalLabelsWidth() == 0
				...
				setLayoutParams(new LayoutParams(
						(int) (textWidth + GraphViewConfig.BORDER),
						LayoutParams.FILL_PARENT));
			...
			int versleft = verlabelsLeft.length - 1;
			for (int i = 0; i < verlabelsLeft.length; i++) {
				float y = ((graphheight / versleft) * i) + border;
				paint.setColor(graphViewStyle.getVerticalLabelsColor());
				canvas.drawText(verlabelsLeft[i], x, y, paint);
			}
//			paint.setTextAlign(Align.RIGHT);
			int versright = verlabelsRight.length - 1;
			for (int i = 0; i < verlabelsRight.length; i++) {
				float y = ((graphheight / versright) * i) + border;
				paint.setColor(graphViewStyle.getVerticalLabelsColor());
				canvas.drawText(verlabelsRight[i],x+graphwidth, y, paint);
			}
		}
	}
	float height;
	...
	private String[] verlabelsLeft;
	private String[] verlabelsRight;
	...
	private boolean staticVerticalLeftLabels;
	private boolean staticVerticalRightLabels;
	public double graphwidthPers;
	...
	public void setGraphWidthPers(double pers) {
		graphwidthPers = pers;
	}
	...
	protected void drawLegend(Canvas canvas, float horstart) {
		int shapeSize = 15;

		// rect
		paint.setARGB(255, 0, 0, 0);
		float legendHeight = (shapeSize + 5) * graphSeries.size() + 5;
		float lLeft = horstart;
		float lTop;
		switch (legendAlign) {
		case TOP:
			lTop = 10;
	...
	abstract public void drawSeries(Canvas canvas,
	 ...
	 *            if null, labels were generated automatically
	 */
	public void setVerticalLeftLabels(String[] verlabelsLeft) {
		staticVerticalLeftLabels = verlabelsLeft != null;
		this.verlabelsLeft = verlabelsLeft;
	}
	 ...
	 *            if null, labels were generated automatically
	 */
	public void setVerticalRightLabels(String[] verlabelsRight) {
		staticVerticalRightLabels = verlabelsRight != null;
		this.verlabelsRight = verlabelsRight;
	}
